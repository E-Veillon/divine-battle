"""
All possible effects a card can have in the game.
"""

from base import EffectNames


class DirectEffects(EffectNames):
    ALL_INCLUSIVE = (
...
class Restrictions(EffectNames):
    NOT_ALONE = (
    ...
    REPLACEABLE = (
        "Cette carte peut être échangée par un autre joueur que le propriétaire "
        "avec la carte qu'elle remplace. Elle est alors placée dans sa réserve. "
        "Cette action peut être réalisée après l'étape de pioche, ne compte pas "
        "comme une pose ou une complétion, et ne donne pas droit à piocher une "
        "nouvelle carte majeure."
    )
    ANTI_ROYALIST = (
...
class Interactions(EffectNames):
    ...


class EffectNamesPack:
    """
    Pack of Enums of effects a card can have in the game.
    An internal Enum's member name is an effect name and its value is a
    description of its behavior.
    """
    def __init__(self) -> None:
        self.direct_effects = DirectEffects
        self.restrictions = Restrictions
        self.interactions = Interactions

    def __getitem__(self, key: str) -> type[EffectNames]:
        match key.lower():
            case "direct_effect":
            ...
            case str():
                fams_str = ', '.join(
                    [f"{effect_type.name!r}" for effect_type in self.effects_types]
                )
                raise KeyError(
    ...
    @property
    def effects_types(self) -> tuple[type[EffectNames], ...]:
        "Tuple of effect types classes."
        return (
            self.direct_effects,
            self.restrictions,
            self.interactions
    ...
    def get_all_names(self) -> list[str]:
        """Get the names of all effects in the game."""
        return (
            [effect.name for effect in self.direct_effects] +
            [effect.name for effect in self.restrictions] +
            [effect.name for effect in self.interactions]
        )
    
    def get_all_descriptions(self) -> list[str]:
        """Get the description of all effects in the game."""
        return (
            [effect.value for effect in self.direct_effects] +
            [effect.value for effect in self.restrictions] +
            [effect.value for effect in self.interactions]
        )

    def contains(self, name: str | EffectNames) -> bool:
        """Whether given name correspond to any effect in the game."""
        return any(effect_type.contains(name) for effect_type in self.effects_types)